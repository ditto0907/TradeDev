...
async def validate_bars(
    ...
    If *fetcher* is provided, uses its fetch_range() (shares the server's
    IB connection — avoids event-loop deadlocks). Otherwise falls back to
    creating a standalone IB connection."""
    if fetcher:
        db_bars = db.get_bars(symbol, timeframe, from_ts, to_ts)
        ib_bars = await fetcher.fetch_range(timeframe, from_ts, to_ts, symbol=symbol)
    else:
        conn_ib, should_disconnect = await _connect_ib(ib)
        try:
            db_bars = db.get_bars(symbol, timeframe, from_ts, to_ts)
            ib_bars = await _fetch_ib_bars(conn_ib, symbol, timeframe, from_ts, to_ts)
        finally:
...
async def fix_bars(
    ...
    insert bars that exist only in IB. Returns summary."""
    if fetcher:
        db_bars = db.get_bars(symbol, timeframe, from_ts, to_ts)
        ib_bars = await fetcher.fetch_range(timeframe, from_ts, to_ts, symbol=symbol)
    else:
...
async def validate_all(
    ...
    so we rate-limit and work in chunks.
